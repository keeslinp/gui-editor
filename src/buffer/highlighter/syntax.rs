...
impl MatchValue {
    fn build(
        ...
        Ok(match raw {
            ...
            MatchActionRaw::List(values) => MatchValue::List(
                values
                    .iter()
                    .filter_map(|value| MatchValue::build(value, variables, anon_contexts).ok())
                    .collect(),
            ),
        })
    }
...
impl Context {
    fn build(
        ...
        let meta_include_prototype = raw.iter().find_map(|segment| {
            if let ContextSegmentRaw::MetaIncludePrototype {
        ...
        let elements = raw
            ...
            .filter_map(|segment| match segment {
                ContextSegmentRaw::Include { include } => {
                    Some(ContextElement::Include(include.clone()))
                }
                ContextSegmentRaw::Match {
                    ...
                    action: match (push, pop, set) {
                        (Some(push), None, None) => Some(MatchAction::Push(
                            MatchValue::build(push, variables, anon_contexts)
                                .expect("Building MatchValue from Raw"),
                        )),
                        (None, Some(_), None) => Some(MatchAction::Pop),
                        (None, None, Some(set)) => Some(MatchAction::Set(
                            MatchValue::build(set, variables, anon_contexts)
                                .expect("Building MatchValue from Raw"),
                        )),
                        (None, None, None) => None,
                        _ => panic!("Only one of push, pop, or set is allowed"),
                    },
                    captures: captures.as_ref().map(|captures| {
                        captures
                            .values()
    ...
    fn try_from(raw: SyntaxRaw) -> Result<Syntax> {
        let (contexts, anon_contexts) = {
            ...
            for (name, context_segments) in raw.contexts.iter() {
                contexts.insert(
                    name.clone(),
                    Context::build(context_segments, &raw.variables, &mut anon_contexts)?,
                );
            }
...
impl Syntax {
    pub fn new(file_extension: &str) -> Result<&'static Syntax> {
        let syntaxes = SYNTAXES.get_or_init(|| {
            RAW_SYNTAXES
                .iter()
                .filter_map(|raw| serde_yaml::from_str::<SyntaxRaw>(raw).ok()?.try_into().ok())
                .collect()
        });
